...
###K&R###
${ID_create_New}=   //a[contains(text(),'Create New')]
${ID_Joural_Type}=   //*[@id="JournalTypeId"]
${ID_Description_journal}=  //textarea[@id='Description']
${ID_Details_journal}=  //textarea[@id='Details']
${Id_create_Button_Journal}=    //input[@type='submit']
...
#Journal Verification For TMS Side Testing
    [Arguments]    ${test_data}
    click element    (//a[text()="View"])[1]
    wait until element is not visible   globalAjaxLoading  100
    ${Journal_DateTime}=    get text    ((//table[@class="DischargedTable"])//tbody//td)[10]
    #log to console    ${Journal_DateTime}
    ${Conv_Date}=    TMS Current Date Conversion
    #log to console     Conv_Date:${Conv_Date}
    ${Date}=    fetch from left    ${Journal_DateTime}    ${space}
    log to console    Journal_Date:${Date}
    run keyword if  '${Conv_Date}' == '${Date}'  log to console    "Journal Date matches and is verified"
    ${Entered_By}=    get text    ((//table[@class="DischargedTable"])//tbody//td)[8]
...
Create new entry in Journal
    ...
    click element    ${UMS-JOURNAL}
    wait until element is not visible   globalAjaxLoading  100
    click element    ${ID_create_New}
    wait until element is not visible   globalAjaxLoading  100
    click element   ${ID_Joural_Type}
    Check and select from list by label    ${ID_Joural_Type}    ${test_data['Journal Type']}
    Check and enter input    ${ID_Description_journal}  ${test_data['Description']}
    Check and enter input    ${ID_Details_journal}  ${test_data['Details']}
    click element    ${Id_create_Button_Journal}
