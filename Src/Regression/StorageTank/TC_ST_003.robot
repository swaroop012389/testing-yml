...
TC_Storage_Tank_003
    ...
    ${file_path}=  Set variable  ${config_data['Path']['st']}${/}TC_ST_003.xlsx
    ...
    Set To Dictionary    ${test_data}  TestCaseNumber=TC_ST_003
    run keyword if  '${test_data['Dummy']}' == 'YES'  RE Create Dummy Data  ${test_data}
    Set To Dictionary    ${test_data}  Program=StorageTank
    Run Keyword And Continue On Failure   Upload Schedule single walled Installed prior to 1990  &{test_data}
    @{res_row} =  Create list  TC_ST_003   ${WorkID}  ${policy-no}
    Update resource file  ${res_row}


*** Keywords ***
Upload Schedule single walled Installed prior to 1990
    [Arguments]    &{test_data}
