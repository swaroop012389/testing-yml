...
TC_Storage_Tank_002
    ...
    ${file_path}=  Set variable  ${config_data['Path']['st']}${/}TC_ST_002.xlsx
    ...
    Set To Dictionary    ${test_data}  TestCaseNumber=TC_ST_002
    run keyword if  '${test_data['Dummy']}' == 'YES'  RE Create Dummy Data  ${test_data}
    Set To Dictionary    ${test_data}  Program=StorageTank
    Run Keyword And Continue On Failure   Manual entry Covered Facility and Tank information  &{test_data}
    @{res_row} =  Create list  TC_ST_002  ${WorkID}  ${policy-no}
    Update resource file  ${res_row}


*** Keywords ***
Manual entry Covered Facility and Tank information
    [Arguments]    &{test_data}
