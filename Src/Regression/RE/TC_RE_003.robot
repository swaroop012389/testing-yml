...
Resource  ../../Resource/REO_Res/REO_Holds_List.robot

*** Test Cases ***
TC_RE_003
    [Tags]   Real Estate_Smoke_Test
    [Documentation]    Holds-Holds-Quoted premium greater than $10,000

    #&{TC_INFO}=  RE Setup test data
    &{config_data}=  Read config data  config.txt
    ${file_path}=  Set variable  ${config_data['Path']['re']}${/}TC_RE_003.xlsx
    ...
    Set To Dictionary    ${test_data}  TestCaseNumber=TC_RE_003
    run keyword if  '${test_data['Dummy']}' == 'YES'  RE Create Dummy Data  ${test_data}
    Set To Dictionary    ${test_data}  Program=REALESTATE
    Run Keyword And Continue On Failure   Holds-Holds-Quoted premium greater than $10,000   &{test_data}
    @{res_row} =  Create list  TC_RE_003   ${WorkID}   ${policy-no}
    Update resource file  ${res_row}
    Close Browser

*** Keywords ***
Holds-Holds-Quoted premium greater than $10,000
        [Arguments]    &{test_data}
         ...
         Run Keyword if   ${element-check}    UMS remove hold
