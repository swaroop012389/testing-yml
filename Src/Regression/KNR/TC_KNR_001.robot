...
*** Test Cases ***
TC_KNR_Smoke_Test_001
    [Tags]    KNR_Smoke_Test
    [Documentation]    Basic Policy Creation.
    &{config_data}=  Read config data  config.txt
    ${file_path}=  Set variable  ${config_data['Path']['kr']}${/}TC_K&R_001.xlsx
    ...
    Run Keyword And Continue On Failure  Basic Policy Creation  &{test_data}
    @{res_row} =  Create list  TC_K&R_001   NA  ${policy-no}
