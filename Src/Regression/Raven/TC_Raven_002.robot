...
*** Test Cases ***
TC_Raven_Smoke_Test_002
    [Tags]    Raven_Smoke_Test
    [Documentation]    AOP+EQ+Flood+OtherEQ + Special Hazard Flood policy generation
    &{config_data}=  Read config data  config.txt
    ${file_path}=  Set variable  ${config_data['Path']['raven']}${/}TC_Raven_002.xlsx
...
#    run keyword if  '${test_data['Dummy']}' == 'YES'  RE Create Dummy Data  ${test_data}
    Run Keyword And Continue On Failure  AOP+EQ+Flood+OtherEQ + Special Hazard Flood policy generation  &{test_data}
    @{res_row} =  Create list  TC_Raven_002   NA  ${policy-no}
