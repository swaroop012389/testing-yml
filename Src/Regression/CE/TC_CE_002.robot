...
TC_CE_002
    ...
    [Documentation]    Pieces_1 to 5_Waterborne 2.5K Deductible
    &{config_data}=  Read config data  config.txt
    ${file_path}=  Set variable  ${config_data['Path']['ce']}${/}TC_CE_002.xlsx
    ...
    Set To Dictionary    ${test_data}  TestCaseNumber=TC_CE_002
    run keyword if  '${test_data['Dummy']}' == 'YES'  Create Dummy Data CE  ${test_data}
    Set To Dictionary    ${test_data}  Program=CE
    Run Keyword And Continue On Failure   Pieces_1 to 5_Waterborne 2.5K Deductible   ${test_data}
    @{res_row} =  Create list  TC_CE_002   ${Work ID}  ${test_data['PolicyNumber']}
    Update resource file  ${res_row}
    Close Browser

*** Keywords ***
Pieces_1 to 5_Waterborne 2.5K Deductible
    [Arguments]    ${test_data}
